...
public class MouseOrbitImproved : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Input.getMouseButton(0) && (Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift))) panning = true;
#endif


            if (panning)
            {
                targetOffset.x -= Input.GetAxis("Mouse X");
                targetOffset.y -= Input.GetAxis("Mouse Y");
            }else if(active)
            {
    ...
    {
        Vector3 tPos = target.transform.position + targetOffset;
        Gizmos.color = Color.yellow;
        Gizmos.DrawLine(transform.position, tPos);
        Gizmos.DrawWireCube(tPos, Vector3.one * .05f);
    }
}